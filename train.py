...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input_tl, target) in enumerate(train_loader):
        data_time.update(time.time() - end)
        
        if torch.cuda.is_available():
            target = target.cuda(non_blocking=True)
        input_var = torch.autograd.Variable(input_tl)
        ...
        losses.update(loss.item(), input_tl.size(0))  # input.size(0): Batch size
        top1.update(prec1[0], input_tl.size(0))
        top5.update(prec5[0], input_tl.size(0))
...
def evaluate(test_loader, model, criterion):
    ...
    for i, (input_tl, target) in enumerate(test_loader):
        if torch.cuda.is_available():
            target = target.cuda(non_blocking=True)
        input_var = torch.autograd.Variable(input_tl, volatile=True)
        ...
        # measure accuracy and record loss
        prec1, prec5 = accuracy(output.data, target, topk=(1, 5))
        losses.update(loss.item(), input_tl.size(0))
        top1.update(prec1[0], input_tl.size(0))
        top5.update(prec5[0], input_tl.size(0))
